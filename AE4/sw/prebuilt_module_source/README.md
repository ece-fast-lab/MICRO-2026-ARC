...
This directory preserves the source and Makefiles that accompanied the two prebuilt modules from the original SPR1 setup.
These files are provenance material only.
Normal setup loads `sw/kmod_pgmigrate/page_migrate.ko` and `sw/kmod_pac_ofw_buf/pac_ofw_buf.ko`.
Fallback builds use the maintained source in those module directories instead of this snapshot.
...
Absolute paths in DWARF debug sections record build provenance and are not runtime dependencies.
