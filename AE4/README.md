...
AE4 reproduces Figure 11 for GAPBS `bc_tw`, `bfs_tw`, and `pr_tw`.
It compares CXL-only, CHMU-Cache, CHMU-CMS, and Adaptive policies at threshold 16.
...
- SPR1 with the CHMU FPGA card and access to the FPGA programming server
...
Run setup and benchmarks as the reviewer account.
The scripts use `sudo` for privileged operations when required.
...
Ask the authorized system operator to power-cycle SPR1.
BMC credentials and the power-cycle command are supplied separately.

After SPR1 boots, reconnect and verify the kernel and NUMA nodes:
...
The setup detects the SPR1 platform, loads the bundled kernel modules, builds the threshold managers, and applies the experiment settings.
...
Normalized performance is `CXL-only geometric-mean time / method geometric-mean time`.
Values above `1.0` are faster than CXL-only.
Adaptive reports the faster complete result from the two initial epoch orders for each workload.
