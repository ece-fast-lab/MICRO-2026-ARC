...
check_requirements() {
    ...
    local required_commands=(
        bash cmake make gcc g++ python3 lspci setpci numactl numastat taskset perf
    ...
    if command -v python3 >/dev/null 2>&1 && \
       python3 -c 'import matplotlib' >/dev/null 2>&1; then
        ...
        warn "Python matplotlib is missing; Figure 4 plotting requires it"
        missing=1
    fi
