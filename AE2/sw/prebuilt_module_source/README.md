...
This directory contains the legacy source and Makefiles that accompanied the two prebuilt modules from the original SPR1 setup.
These files are retained for provenance only.
Normal setup loads the validated modules from `sw/kmod_pgmigrate/page_migrate.ko` and `sw/kmod_pac_ofw_buf/pac_ofw_buf.ko`.
Fallback builds use the maintained source in those module directories rather than this snapshot.
...
Setup therefore accepts the bundled pair only for the original SPR1 node layout.
Historical absolute paths in DWARF debug sections are build provenance and are not runtime dependencies.
