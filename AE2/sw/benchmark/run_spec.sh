...
if [[ $# -lt 7 ]]; then
  echo "Usage: $0 <cnt_th> <epoch_cycle_a> <epoch_cycle_b> <poll_ms> <spec> <copies> <mode> [tag]"
  exit 1
fi
...
#   ./run_spec.sh <cnt_th> <epoch_cycle_a> <epoch_cycle_b> <poll_ms> <spec> <copies> <mode> [tag]
#
# mode:
...
RUN_OUTPUT_POLICY="${RUN_OUTPUT_POLICY:-rotate}"
...
[[ "${spec}" =~ ^[0-9]+$ ]] || \
  { echo "ERROR: spec must be a numeric benchmark identifier such as 502"; exit 1; }
[[ "${copies}" =~ ^[1-9][0-9]*$ ]] || \
  { echo "ERROR: copies must be a positive decimal integer"; exit 1; }
[[ "${tag}" =~ ^[A-Za-z0-9._-]+$ ]] || \
...
[[ "${MIGRATION_START_DELAY_SEC:-0}" =~ ^[0-9]+$ ]] || \
  { echo "ERROR: MIGRATION_START_DELAY_SEC must be a nonnegative decimal integer"; exit 1; }

case "$RUN_OUTPUT_POLICY" in
  rotate|backup|fail) ;;
  *) echo "ERROR: RUN_OUTPUT_POLICY must be one of: rotate | backup | fail"; exit 1 ;;
esac
...
SET_SPEC_DIR="${SPEC_ROOT:?Set SPEC_ROOT in ${BENCHMARK_PATHS_FILE}}"
SPEC_BIN="${SPEC_RUNCPU:-${SET_SPEC_DIR}/bin/runcpu}"
SPEC_CONFIG="${SPEC_CONFIG:-${SET_SPEC_DIR}/config/myprogram-gcc-linux-x86.cfg}"
...
MIGRATION_START_DELAY_SEC="${MIGRATION_START_DELAY_SEC:-0}"
...
backup_run_output() {
  ...
  while [[ -e "${run_dir}${suffix}" || -L "${run_dir}${suffix}" || \
           -e "${run_log}${suffix}" || -L "${run_log}${suffix}" ]]; do
    generation=$((generation + 1))
    suffix=".bak.${generation}"
  done

  if [[ -e "$run_dir" || -L "$run_dir" ]]; then
    mv -- "$run_dir" "${run_dir}${suffix}"
    echo "[output] moved existing run directory to ${run_dir}${suffix}"
  fi
  if [[ -e "$run_log" || -L "$run_log" ]]; then
    mv -- "$run_log" "${run_log}${suffix}"
    echo "[output] moved existing run log to ${run_log}${suffix}"
  fi
}
...
RUN_OUT_DIR="${OUT_BASE_DIR}/${cnt_th}_${epoch_cycle_a}_${epoch_cycle_b}_${poll_ms}_${spec}_${mode}_${tag}"
LOG_FILE="${RUN_OUT_DIR}.log"
if [[ "$RUN_OUTPUT_POLICY" == "fail" ]]; then
  if [[ -e "$RUN_OUT_DIR" || -L "$RUN_OUT_DIR" || -e "$LOG_FILE" || -L "$LOG_FILE" ]]; then
    echo "ERROR: output already exists and RUN_OUTPUT_POLICY=fail: $RUN_OUT_DIR" >&2
    exit 1
  fi
elif [[ "$RUN_OUTPUT_POLICY" == "backup" ]]; then
  backup_run_output "$RUN_OUT_DIR" "$LOG_FILE"
else
...
echo "mode=$mode spec=$spec copies=$copies tag=$tag"
echo "epoch_cycle_a=$epoch_cycle_a epoch_cycle_b=$epoch_cycle_b poll_ms=$poll_ms"
echo "SRC_NODE=$SRC_NODE DST_NODE=$DST_NODE"
echo "WL_CPUS=$WL_CPUS MIGRATION_CPU=$MIGRATION_CPU PHASE1_SEC=$PHASE1_SEC"
echo "MIGRATION_START_DELAY_SEC=$MIGRATION_START_DELAY_SEC"
echo "ENABLE_DEBUG_MONITOR=$ENABLE_DEBUG_MONITOR DBG_INTERVAL_SEC=$DBG_INTERVAL_SEC"
...
[[ -d "$SET_SPEC_DIR" ]] || { echo "ERROR: SET_SPEC_DIR not found: $SET_SPEC_DIR"; exit 1; }
[[ -x "$SPEC_BIN" ]] || { echo "ERROR: SPEC_BIN not executable: $SPEC_BIN"; exit 1; }
[[ -r "$SPEC_CONFIG" ]] || { echo "ERROR: SPEC_CONFIG not readable: $SPEC_CONFIG"; exit 1; }
...
debug_monitor() {
  ...
  while true; do
    ...
    echo "[debug] mode=$mode spec=$spec copies=$copies" >> "$DBG_LOG"
    ...
    if command -v numastat >/dev/null 2>&1; then
      echo "--- numastat -c base ---" >> "$DBG_LOG"
      LC_ALL=C numastat -c base >> "$DBG_LOG" 2>/dev/null || true
    fi
...
echo "[2/4] Start SPEC workload INSIDE cgroup (fail-fast join verification)"
SPEC_LOG="${RUN_OUT_DIR}/${spec}.log"
rotate_if_exists "$SPEC_LOG"
...
taskset -c "$WL_CPUS" bash -lc "
  ...
  if [[ '${mode}' == 'baseline' && '${BASELINE_MEM_POLICY}' == 'interleave' ]]; then
...
if ! pid_is_running_non_zombie "$WORKLOAD_PID"; then
  echo "ERROR: workload died immediately. Tail of SPEC log:"
  tail -n 120 "$SPEC_LOG" || true
  exit 1
fi
echo

if [[ "$mode" == "mig" ]]; then
  echo "[3/4] Pre-migration CXL-only warmup: ${MIGRATION_START_DELAY_SEC}s"
  if (( MIGRATION_START_DELAY_SEC > 0 )); then
    sleep "$MIGRATION_START_DELAY_SEC"
    if ! pid_is_running_non_zombie "$WORKLOAD_PID"; then
      echo "ERROR: workload exited during the pre-migration warmup. Tail of SPEC log:"
      tail -n 120 "$SPEC_LOG" || true
      exit 1
    fi
  fi
  ...
  echo "[3/4] Pre-migration warmup: not used for mode=$mode"
  echo
fi
...
case "$mode" in
  ...
  mig)
    ...
    echo "migration_manager mode switch epochs: mode0=${epoch_cycle_a} mode1=${epoch_cycle_b}"
...
{
  echo "mode=$mode spec=$spec copies=$copies"
  echo "threshold=$cnt_th epoch_cycle_a=$epoch_cycle_a epoch_cycle_b=$epoch_cycle_b poll_ms=$poll_ms tag=$tag"
  echo "MIGRATION_START_DELAY_SEC=$MIGRATION_START_DELAY_SEC ENABLE_DEBUG_MONITOR=$ENABLE_DEBUG_MONITOR DBG_INTERVAL_SEC=$DBG_INTERVAL_SEC"
...
if [[ "$work_rc" != 0 ]]; then
  echo "ERROR: SPEC workload failed with rc=${work_rc}" >&2
fi
